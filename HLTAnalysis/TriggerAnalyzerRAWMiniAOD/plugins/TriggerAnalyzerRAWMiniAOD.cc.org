...
TriggerAnalyzerRAWMiniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   //Exercise 3: uncomment the two lines above and modify the input tag to print all trigger objects corresponding to the last filter of the HLT_Mu3_PFJet200CSV_1p5 path (btagged jet with pt>200 GeV)
   ...
   h_mu3pfjet200csv1p5_vs_leadbjetpt_den->Fill(leadingbjetpt);
   if(passHLT_Mu3_PFJet200CSV_1p5) h_mu3pfjet200csv1p5_vs_leadbjetpt_num->Fill(leadingbjetpt);
   if(passHLT_Mu3_L1SingleJet180) h_mu3pfjet200csv1p5_vs_leadbjetpt_numl1->Fill(leadingbjetpt); //For L1 turn on only

   //Effcy vs the highest csv of jet with pt>250:
   h_mu3pfjet200csv1p5_vs_highestcsv_jetpt250_den->Fill(highestcsv_jetpt250);
   if(passHLT_Mu3_PFJet200CSV_1p5) h_mu3pfjet200csv1p5_vs_highestcsv_jetpt250_num->Fill(highestcsv_jetpt250);


   //Effcy vs nb of bjets with pt>200 
   h_mu3pfjet200csv1p5_vs_nbjetspt200_den->Fill(nbjetspt200);
   if(passHLT_Mu3_PFJet200CSV_1p5) h_mu3pfjet200csv1p5_vs_nbjetspt200_num->Fill(nbjetspt200);

   //Effcy vs leading muon pt:
   h_mu3pfjet200csv1p5_vs_leadingmuonpt_den->Fill(leadingmuonpt);
   if(passHLT_Mu3_PFJet200CSV_1p5) h_mu3pfjet200csv1p5_vs_leadingmuonpt_num->Fill(leadingmuonpt);
...
bool TriggerAnalyzerRAWMiniAOD::PassOfflineElectronSelection(const pat::Electron * ele, reco::Vertex::Point PV){
  ...
  if(TMath::Abs(ele->superCluster()->eta()) >2.5) return false;
    ...
    if(gsfTrack->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS)>2) return  false;
  }
  else {
